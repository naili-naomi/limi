...
class LikeRepository {
    ...
    fun addLike(userId: Int, reviewId: Int): Boolean = transaction {
        val existing = ReviewLikes.select(
            (ReviewLikes.user eq userId) and (ReviewLikes.review eq reviewId)
        ).count()
    ...
    fun removeLike(userId: Int, reviewId: Int): Boolean = transaction {
        ReviewLikes.deleteWhere {
            (user eq userId) and (review eq reviewId)
        } > 0
    }
